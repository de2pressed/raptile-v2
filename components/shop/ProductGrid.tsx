...
export function ProductGrid({ products }: ProductGridProps) {
  ...
  const gridColumns = products.length === 1 ? "grid-cols-1" : "grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4";

  return (
    <LazyMotion features={domAnimation}>
      <div className={cn("grid gap-x-4 gap-y-8 md:gap-x-5 md:gap-y-10", gridColumns)}>
